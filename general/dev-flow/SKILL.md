...
1. **不要手动管理调用关系** - dev-flow 会自动决定何时调用其他技能
2. **不要跳过测试** - dev-flow 强制要求测试验证
3. **不要忽略日志** - `dev-flow.log` 包含完整的执行过程
4. **不要过度拆分** - first-principles 会判断是否需要拆解
