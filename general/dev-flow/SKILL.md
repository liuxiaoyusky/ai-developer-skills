...
# 3.5 任务识别失败处理（⚠️ 关键修复）
如果有待处理任务但无法识别：
  AI: "⚠️  任务识别警告：检测到 tasks.md 中有待处理任务，但无法识别下一个合适的任务
  ...
  继续尝试：选择第一个格式正确的待处理任务
  如果仍然失败 → 输出 <WARN>NO_VALID_TASKS_FOUND</WARN> 并结束本次执行
...
第一性原理分析：
  ...
  ↓
⚠️ 错误处理：
  如果 first-principles 调用失败或返回空响应：
    AI: "⚠️  任务分析警告：first-principles 技能调用失败

         任务描述：<任务标题>
         错误原因：<具体错误信息>

         💡 Fallback 策略：
         - 使用基础拆解方案（不调用 first-principles）
         - 按照任务描述直接生成 2-3 个逻辑子任务
         - 继续执行流程

         正在生成 fallback 拆解方案..."
    继续执行：生成基础 task-details.md
  ↓
生成 3-7 个子任务（按依赖关系排序）
  ↓
写入 task-details.md
...
```markdown
## 4.1 调用 dev-verify 技能（自动执行）

进入测试验证阶段，自动调用 dev-verify 技能：

AI: "📊 进入测试验证阶段 (Step 4)...
     正在调用 dev-verify 技能执行所有测试..."

**dev-verify 将自动执行**：
1. 应用 Linus Torvalds 5层代码审查
2. 检测测试框架（pytest/jest/go test/JUnit等）
3. **真实执行测试**（通过 Bash 工具，非 AI 模拟）
...
**循环**：
- dev-verify 持续迭代直到所有测试通过
- 每个失败都通过 dev-debug 修复后重新测试
- 所有测试通过 → 返回场景 A
...
- ❌ AI 模拟测试结果（必须真实执行）
- ❌ 跳过失败测试（必须修复）
- ❌ 禁止等待用户确认 - dev-verify 自动执行
- ❌ 禁止未测试就标记完成 - 必须等待 dev-verify 完成
...
- **自动调用**：Step 4 到达时自动调用 dev-verify
- **真实测试**：通过 Bash 工具执行真实测试命令
- **自动调试**：测试失败时 dev-verify 自动调用 dev-debug
- **结果标记**：所有测试结果自动标记到 task-details-test.md
- **持续迭代**：直到所有测试通过才继续
