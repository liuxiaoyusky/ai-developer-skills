---
name: skill-checker
description: Claude Skills管理工具 - 检查并列出本地skills和marketplace插件，检测插件更新并提示升级，记录和统计使用历史。触发场景："检查技能更新"、"skill更新"、"插件升级"、"查看已安装skills"、"我的常用技能"、"my-skills"。
---
...
> - ⭐ **我的常用**："我的skills"、"常用技能"、"使用记录"、"my-skills"
>
> **功能**：
...
│   └── cache/                     # 插件安装缓存
│
├── skills-usage.json              # Skill使用历史记录（可选）
│
...
My Skills 功能帮助你追踪和发现最常用的 skills，让你能够快速找到和启动使用频率高的技能。
...
```bash
# 显示常用skills
python3 scripts/check_skills.py --my-skills

# 记录使用的skill
python3 scripts/check_skills.py --record <skill-name>

# 记录并指定marketplace
python3 scripts/check_skills.py --record <skill-name> --marketplace ai-developer-skills
```
...
```json
{
  "skill-name@marketplace": {
    ...
    "first_used": "2026-02-01T10:00:00",
    "last_used": "2026-02-04T15:30:00",
    "use_count": 42
  }
}
```
...
```bash
# 查看原始数据
cat ~/.claude/skills-usage.json

# 备份数据
cp ~/.claude/skills-usage.json ~/skills-usage-backup.json

# 清空历史
rm ~/.claude/skills-usage.json

# 编辑记录（手动添加/删除）
nano ~/.claude/skills-usage.json
```
...
```bash
python3 scripts/check_skills.py --record first-principles --marketplace ai-developer-skills
```
