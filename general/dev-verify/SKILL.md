...
AI: "❌ dev-verify 报告：部分测试失败
     ...
     dev-verify 已自动调用 dev-debug 处理每个失败
...
**⚠️ 严禁行为**：
- ❌ 手动运行测试命令（必须使用 dev-verify）
- ❌ AI 模拟测试结果（必须真实执行）
- ❌ 跳过失败测试（必须修复）
- ✅ dev-verify 会持续迭代直到所有测试通过
```
...
- **dev-flow**: 任务管理和 5步工作流（调用 dev-verify）
- **dev-debug**: 5 Whys + 第一性原理调试（被 dev-verify 调用）
